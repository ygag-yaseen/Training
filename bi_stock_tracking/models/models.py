...
# -----Products
    ...
    def action_generate_backorder_wizard(self):
        view = self.env.ref('bi_stock_tracking.my_backorder_view')
        wiz = self.env['my.backorder']
        # .create({'pick_ids': [(4, p.id) for p in self]})
        return {
            'name': _('Create Backorder?'),
            'type': 'ir.actions.act_window',
            'view_mode': 'form',
            'res_model': 'my.backorder',
            'views': [(view.id, 'form')],
            'view_id': view.id,
            'target': 'new',
            'res_id': wiz.id,
    ...
    def no_backorder(self):
        bi_stock_tracking_id = self.env[self.env.context['active_model']].browse(self.env.context['active_ids'])
        location_id = self.env['stock.warehouse'].search([('id','=',bi_stock_tracking_id.picking_from_id.id)])
        record = self.env['stock.picking']
        new_lines =[]
        for data in bi_stock_tracking_id.order_line_ids:
            available = data.product_id.with_context({'location': location_id.lot_stock_id.id}).sudo().qty_available
            new_lines.append((0, 0, {
                'product_id'            :data.product_id.id,
                'product_uom_qty'       :available,
                'description_picking'   :bi_stock_tracking_id.description_picking,
        ...
        pick = {}
        pick = {
            'picking_type_id'           :bi_stock_tracking_id.operation_type_id.id,
            'name'                      :record.product_id.display_name,
            'location_dest_id'          :bi_stock_tracking_id.transit_id.id,
            'location_id'               :location_id.lot_stock_id.id,
            'move_ids_without_package'  :new_lines,
            'move_type'                 :bi_stock_tracking_id.move_type,
            'company_id'                :bi_stock_tracking_id.company_id.id,
            'scheduled_date'            :bi_stock_tracking_id.scheduled_date,
            'origin'                    :bi_stock_tracking_id.sequence_no
        ...
        button_confirm = self.env['stock.immediate.transfer']
        button_confirm.process()

        for data in bi_stock_tracking_id.order_line_ids:
            available = data.product_id.with_context({'location': location_id.lot_stock_id.id}).sudo().qty_available

            move = self.env['stock.move'].create({
            'name'              :bi_stock_tracking_id.sequence_no,
            'location_id'       :location_id.lot_stock_id.id,
            'location_dest_id'  :bi_stock_tracking_id.transit_id.id,
            'product_id'        :data.product_id.id,
            ...
            move._action_assign()
            move.move_line_ids.write({'qty_done':available}) 
            move._action_done()

        bi_stock_tracking_id.write({
        'state': 'reserved',})
    ...
    def validate(self):
        ...
        for data in self.order_line_ids:
            active_qty = self.env['stock.picking'].search([('origin','=',self.sequence_no)])
            for each in active_qty:
                qty = active_qty.move_line_ids_without_package.product_uom_qty
...
class MyBackorder(models.TransientModel):
    _name = 'my.backorder'
    _description = 'Backorder Confirmation'

    pick_ids = fields.Many2many('bi.stock.tracking', 'stock_picking_backorder_rel')
    ...
    def process_cancel_backorder(self):
        record = self.env['bi.stock.tracking']
        record.no_backorder()
